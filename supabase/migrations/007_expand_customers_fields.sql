/*
  ...
  ### Financial Fields
  - `saldo` (decimal) - Current balance
  - `achterstallig` (decimal) - Overdue amount
  - `achterstallig_in_valuta` (decimal) - Overdue in foreign currency
  - `kredietlimiet` (decimal) - Credit limit

  ### Address Fields (multiple addresses)
  - `adres_1`, `adres_2`, `adres_3` (text) - Address lines
  - `afleveradres_1`, `afleveradres_2`, `afleveradres_3` (text) - Delivery address lines
  - `postcode_voor_levering` (text) - Delivery postal code
  - `plaats_voor_levering` (text) - Delivery city
  - `delivery_address` (text) - Alternative delivery address
  - `sleutelcode` (text) - Key code for delivery

  ### Contact Fields
  - `contact` (text) - Primary contact name
  - `email_voor_contact` (text) - Contact email
  - `www` (text) - Website URL
  - `mobiele_telefoon` (text) - Mobile phone
  - `email_voor_factuur` (text) - Invoice email
  - `email_verzenden` (text) - General email for sending
  - `e_factuur` (text) - E-invoice address
  - `contact_voor_levering` (text) - Delivery contact
  - `te_leveren_telefoon` (text) - Delivery phone
  - `te_leveren_email` (text) - Delivery email

  ### Business Configuration
  - `crm_groep` (text) - CRM group
  - `betaling` (text) - Payment terms
  - `betalingsformaat` (text) - Payment format
  - `btw_nummer` (text) - VAT number
  - `prijslijst` (text) - Price list reference
  - `ons_rekeningnummer` (text) - Our account number
  - `filiaal` (text) - Branch
  - `geblokkeerd` (boolean) - Blocked status
  - `bankrekening` (text) - Bank account
  - `transportmethode` (text) - Transport method
  - `leveringsvoorwaarde` (text) - Delivery terms
  - `kvk_nummer` (text) - Chamber of Commerce number
  - `gemaakt` (timestamptz) - Created date from Uniconta
  - `bedrijfsstatus` (text) - Company status
  - `weekdagen` (text) - Delivery days
  - `automatische_orderbevestiging` (boolean) - Auto order confirmation

  ### Timing
  - `bezogtijden` (text) - Visit times
  - `weekfactuur` (text) - Weekly invoice schedule

  ### Additional
  - `account_manager` (text) - Account manager name
  - `link_naar_payt` (text) - Link to payment system
  - `uniconta_synced_at` (timestamptz) - Last sync timestamp from Uniconta

  ## Notes
  - All new fields are optional (nullable) to support gradual data migration
  - Existing customer records remain unchanged
  - Fields match Uniconta column names for easy mapping
*/
